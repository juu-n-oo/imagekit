...
- **이미지 메타데이터 라벨(OCI 표준)** — 빌드한 이미지에 "누가·어떤 Dockerfile·어떤 버전으로 만들었는지" 등 식별·이력 정보를 **이미지 라벨**로 새겨, 이미지만 봐도 출처를 추적할 수 있게 한다. 사용자가 직접 입력하는 값(버전·작성자 등)은 **Dockerfile 안에 기록**되어 함께 보존되고, 빌드할 때마다 자동으로 채워지는 값(생성 시각·리비전 등)은 **빌드 시 주입**된다. 입력은 Dockerfile 에디터의 "이미지 메타데이터" 섹션(접이식)에서 받는다. → 라벨 종류는 아래 참고 박스 참조.
