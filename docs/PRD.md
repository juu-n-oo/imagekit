...
- **Volume 웹 파일 업로드** — Beta에서 제한(비활성화). exec stdin 불안정으로 SFTPServer 방식 재설계 예정 (§10 구현 결정 사항 · §11 Open Questions 참조)
- 보안 취약점 스캔
...
설계서에 없던, 구현을 진행하며 자체적으로 결정·확정한 사항을 기록한다. 출시 전 추가 검토·합의가 필요한 항목은 §11 Open Questions로 승격한다.
...
- **Volume 웹 파일 업로드 제한** — 브라우저에서 Volume에 파일을 직접 업로드하는 기능은 Beta에서 비활성화한다. 백엔드가 PVC를 마운트한 helper Pod에 `kubectl exec`로 `dd`를 실행해 multipart를 **stdin**으로 흘려보내는 방식인데, 이 클러스터의 exec stdin 채널(konnectivity/WebSocket 경유)이 불안정하여 파일 크기와 무관하게 `Broken pipe`로 즉시 끊긴다. 파일 목록 *조회*(exec stdout)와 **기존 Volume 파일을 선택**해 빌드 컨텍스트로 쓰는 경로는 정상 동작하므로 유지한다. 업로드는 SFTPServer 기반으로 재설계 예정(§11 참조).
