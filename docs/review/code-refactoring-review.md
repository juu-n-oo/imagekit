...
### SRV-5 — `ApiException→RuntimeException` 변환 로직 중복 · **Medium** · ✅ 완료
> ✅ **완료(2026-06-11)**: `K8sExceptions.translate/translateNon404` 헬퍼로 ApiException 변환 일원화, `ImageBuildService` 적용. (Volume 클라이언트 적용은 후속)
...
### SRV-6 — `findBuildPodName` 이 Job 이름·라벨을 직접 재유도 · **Medium** · ✅ 완료
> ✅ **완료(2026-06-11)**: `findBuildPodName` 을 `Optional` 반환 + 컨트롤러 소유 라벨 `aipub.ten1010.io/imagebuild-name` 으로 Pod 선택. (명명 헬퍼 공유는 제외 결정)
...
### SRV-7 — `getBuildLogs` 가 분기를 예외로 처리 · **Medium** · ✅ 완료
> ✅ **완료(2026-06-11)**: `getBuildLogs` 를 throw-then-catch 없이 선형 OpenSearch fallback 으로 재작성.
...
### SRV-8 — `logStreamExecutor` 무제한 cached pool · 미종료 · **Medium** · ✅ 완료
> ✅ **완료(2026-06-11)**: SSE executor 를 무제한 cached pool → 상한(32) bounded pool + `@PreDestroy` 종료로 교체.
...
### SRV-9 — 레지스트리 파서의 무타입 Map · **Medium** · ✅ 완료
> ✅ **완료(2026-06-11)**: 외부 레지스트리(NGC/HuggingFace) 기능 자체를 제거(패키지·application.yaml·helm env 삭제)하여 해소.
...
### SRV-12 — MCP actor 귀속 비일관 (관련 주석은 본 패스에서 수정) · **Low** · 🟢 부분완료
> 🟢 **부분완료(2026-06-11)**: `ImageBuildController` 의 오래된 MCP 주석을 정정. MCP actor 귀속(`"mcp-tool"` 고정) 통일은 미시행.
...
### SRV-13 — `AipubAuthenticationFilter` 광범위 catch (인가 후순위 — 참고만) · **Low** · ✅ 완료
> ✅ **완료(2026-06-11)**: 인증 필터에서 4xx(무효 쿠키=미인증 continue)와 5xx·연결오류(503 으로 빠르게 실패)를 분리.
